...
from box_sdk_gen import BoxClient, BoxDeveloperTokenAuth, UploadFileAttributes, UploadFileAttributesParentField, CreateFolderParent
...
class AppBoxStorage(Storage):
    def __init__(self):
        self.api = BoxClient(BoxDeveloperTokenAuth(token="TZNxfaXhSKZ9TXAnEIJLbb44D1IMGbis"))
    ...
    def upload_file(self, name, file):
        files = self.api.uploads.upload_file(UploadFileAttributes(name=name, parent=UploadFileAttributesParentField(id=self.folder_id)), file)
